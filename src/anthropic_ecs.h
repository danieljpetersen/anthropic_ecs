...
struct EntityId {
    ...
    bool operator==(const EntityId& other) const {
        return unstableIndex == other.unstableIndex && version == other.version && poolKey == other.poolKey;
    }
};
...
// this results in each pool technically having more vectors than strictly needed, but i'm pretty sure the unused ones are effectively ignored
...
public:
    ...
    template<typename... Components, typename Func>
    void forEachEarlyReturn(Func callback) {
        for (std::size_t i = 0; i < poolSize; ++i) {
            EntityId id;
            id.unstableIndex = i;
            id.version = versions[i];
...
public:
    ...
    void addComponent(EntityId& entityId, const ComponentToAdd& component) {
        ...
        if (resolveEntityId(entityId, oldPool)) {
            if (oldPool->template hasComponent<ComponentToAdd>()) {
                // todo should still set arg component to the entity
                return;
            }
            ...
            if (newPoolIt == pools.end()) {
                ...
                // todo we should change all uses of initFromBitmask to initFromIndices, i think it'd be faster
    ...
    void forEachComponentsEarlyReturn(Func callback) {
        ...
        for (auto& poolPair : pools) {
            ...
            if (pool.template hasComponents<Components...>()) {
                pool.template forEachEarlyReturn<Components...>(callback);
            }
        }
        isIterating = false;
    }
